...
- A 2026-06-19 Android audible speech pass is recorded in `docs/speech-qa-results/2026-06-19-android-audible-transcription.md`: the standalone Pixel 8a app captured spoken audio through hold-to-speak, produced a usable transcript, saved it as a voice memory, preserved it after force-stop/relaunch, found it through search with the keyboard open, and showed no app-process fatal/error entry. Milestone 3 remains open for long-pause, background interruption, iOS, and accepted web permission QA.
...
- Milestone 6: production structured-extraction target selected as `Qwen2.5-0.5B-Instruct` through `llama.rn`; portable runtime adapter, checked asset manifest, guarded engine factory, Expo document-storage asset discovery, native `llama.rn` context loader, guarded user-facing Qwen mode, Settings diagnostic, import/repair flow, and rules-plus-Qwen hybrid suggestion merge are present. Pixel 8a missing-asset fallback UI passed, the actual GGUF asset probe passed on 2026-06-19, and Pixel 8a runtime probes passed on 2026-06-19 with Qwen diagnostics at 7824 ms, 7219 ms, and a later 7698 ms refresh. App-data-clear recovery, golden-memory output-quality QA, and memory-pressure QA remain open.
- Qwen transcript formatting is wired into new-memory and voice-transcript draft review as an optional punctuation/capitalization-only action. Pixel 8a QA for actual-model formatting quality, editable-before-save behavior, and non-destructive rewrite rejection passed on 2026-06-19 in `docs/pixel-8-results/2026-06-19-qwen-transcript-formatting.md`. Missing-asset messaging remains covered by adapter/UI behavior but still needs a manual device recheck after app-data-clear recovery.
- Milestone 7: production embedding target selected as `BAAI/bge-small-en-v1.5` through ONNX Runtime; portable BGE adapter, checked asset manifest with expected sizes/hashes, guarded engine factory, Expo document-storage asset discovery/import repair, tokenizer-json WordPiece parser, ONNX Runtime loader, user-facing guarded BGE mode, and Settings diagnostic are present. Pixel 8a missing-asset fallback UI passed, the actual ONNX/tokenizer asset probe passed on 2026-06-19, and Pixel 8a BGE runtime diagnostics passed on 2026-06-19 with 419 ms and 488 ms probes, 384 dimensions, and vector norm 1.000. Broader embedding rebuild/search ranking, fallback, memory, and app-data-clear recovery QA remain open.
- Milestone 11: WebDAV encrypted sync is the first production sync provider target; Pixel 8a loopback first-push QA passed, and authenticated loopback QA now covers wrong-credential feedback, successful credentialed push, and remote plaintext-leak checks. Broader non-loopback provider, restart/resume, existing-remote pull/merge, and conflict QA remain open.

### Needs Device QA

- Milestone 3: iOS and full web speech-recognition QA, including accepted permission prompts, long pauses, background interruption, and transcription fallback. Web denied-permission fallback evidence is recorded in `docs/speech-qa-results/2026-06-13-web-denied-permission.md`; Android standalone no-speech recovery evidence on the attached Pixel 8a is recorded in `docs/speech-qa-results/2026-06-14-android-voice-standalone.md`; Android audible hold-to-speak transcription evidence is recorded in `docs/speech-qa-results/2026-06-19-android-audible-transcription.md`.
...
- Added a Settings Life Calendar control for birth year/month/day plus school-year start month and kindergarten start age. Age, grade, and calendar year remain interchangeable suggestions rather than rewritten memory text: a full birthday tightens age ranges to birthday-to-birthday spans, grade suggestions use the stored school-year assumptions, and Review/editor rules now receive the archive profile. SQLite now persists the profile on Android, and regression tests cover helper mapping, extraction, review suggestions, and the profile migration.
- Ran Pixel 8a standalone QA for the Life Calendar flow and recorded it in `docs/pixel-8-results/2026-06-19-life-calendar-settings.md`: preflight/build/install passed, Settings accepted `1985-03-12`, the preview mapped age 8 to `1993-03-12 to 1994-03-11` and 3rd grade to `1993-08-01 to 1994-06-30`, force-stop/relaunch preserved the values, and the app-specific log slice showed no fatal exception.
- Clarified the Life Calendar Settings fields with persistent labels and short hints for birth year/month/day, school-year start month, and kindergarten start age. A fresh Pixel 8a standalone reinstall verified the labels and existing saved birthday data, with no app fatal exception in the checked log slice.
- Added app-wide button acknowledgement and keyboard-safe editing behavior, with Pixel 8a QA recorded in `docs/pixel-8-results/2026-06-19-button-feedback-keyboard-visibility.md`: shared buttons now use pressed visual feedback and short Android vibration, Settings personal-information saves show progress/success/error text, Android resizes around the keyboard so focused inputs remain visible, profile-only saves skip embedding maintenance, and Android SQLite rewrites clear the prior `user_profile` row before reinserting it.
...
- Fixed native ONNX Runtime availability for standalone Android by removing the package's Expo unimodule marker through the tracked patch, explicitly wiring the Android Gradle project, registering `OnnxruntimePackage`, and adding a preflight native-module guard before importing the JavaScript runtime.
- Converted BGE ONNX feeds to native `int64` tensors at runtime. The Pixel 8a standalone `Test BGE` diagnostic passed with actual assets in 539 ms, 384 vector dimensions, and vector norm 1.000.
...
- Added a Qwen transcript-formatting adapter and `Format transcript` actions in typed new-memory capture and voice transcript review. The prompt asks Qwen to add punctuation and capitalization only, preserve original words, avoid summaries or added facts, and return plain text; the editable draft is replaced only after a successful model response, missing Qwen assets show a non-destructive readiness message, and no rules-based fallback formatting is applied. Pixel/manual QA for this flow remains required.
...
- Pixel 8a standalone archive-at-rest migration/restart-unlock passed: archive scope saved with a temporary passphrase, relaunch required the encrypted archive unlock screen, correct passphrase unlocked the app, and cleanup restored a non-locked launch.
...
- Pixel 8a standalone authenticated WebDAV loopback evidence for wrong-credential feedback, successful credentialed push, encrypted remote record format, and no checked plaintext memory tokens in the stored payload.

Remaining:
- Broader WebDAV provider QA for non-loopback servers, restart/reconnect behavior, existing-remote pull/merge behavior, conflicts, and any cloud-AI adapters behind explicit consent.
