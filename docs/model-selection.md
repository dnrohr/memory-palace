...
- Non-purpose: transcript formatting. BGE is an embedding model for vector search and relatedness, not a text-generation model.
...
- Secondary purpose: optional transcript formatting in capture drafts. The `Format transcript` flow uses Qwen only when the local GGUF asset is ready, asks for punctuation and capitalization only, and leaves the formatted draft editable before save.
- Current fallback: rules-based structured extraction.
- Transcript-formatting fallback: none. If Qwen is unavailable, the app reports that the Qwen local model is not ready and does not apply rules-based punctuation or capitalization.
- Integration posture: JSON-speaking structured extraction adapter, transcript-formatting adapter, and Qwen/llama runtime adapter are present. A checked asset manifest and factory require the quantized GGUF file before creating Qwen-backed engines, with optional grammar support used only by structured extraction. The Expo app checks for these files under `models/qwen2.5-0.5b-instruct` in app document storage and has a native `llama.rn` loader that initializes a context from the resolved GGUF URI when Qwen local mode is selected or transcript formatting is requested. Local-model JSON is normalized conservatively before validation so string tags and loose date candidates can be accepted with defaults instead of crashing the probe. Qwen mode now merges useful model candidates into local rules output instead of replacing rules, and filters low-confidence filler, pronoun, standalone-year, and ordinary-verb tags. Model weights are not bundled in the repository; Settings can reimport the GGUF artifact after app-data clear.
...
- QA expectations: Qwen structured-extraction mode must not regress rules tags. Probe success proves the runtime loads, but acceptance is based on golden tag quality: rules baseline suggestions must remain present, and model-added pronouns, ordinary verbs, and standalone years must be filtered. Transcript-formatting acceptance is narrower: the formatted draft may add punctuation and capitalization only, must not summarize, add facts, change names/dates/places, or rewrite meaning, and must remain user-editable before save.
...
- Keep rules as the default for structured extraction until device QA confirms document-picker model-file import/repair after app-data clear and broader Pixel 8a QA with actual GGUF assets confirms acceptable repeated-run latency, memory use, recovery behavior, structured-output quality, and transcript-formatting preservation.
